...
# Load a lightweight sentiment model (downloads once, cached)
# Using cardiffnlp/twitter-roberta-base-sentiment-latest for YouTube comment style text
...
def clean_text(text):
    ...
    return text[:512]  # Truncate for model


def generate_overall_feedback(pos_pct, neg_pct, neu_pct, top_positive, top_negative):
    """Generate a human-readable overall feedback summary."""
    dominant = max(
    ...
    if dominant == "positive":
        ...
        tone = "mostly critical" if neg_pct > 50 else "mixed with more criticism"
    else:
...
def analyze():
    ...
    for item in raw_results:
        # pipeline returns a list of dicts when top_k=1
    ...
    # Find representative comments for summary
    ...
    avg_positive_score = round(sum(scores.get("positive", [0])) / max(len(scores.get("positive", [1])), 1), 3)
